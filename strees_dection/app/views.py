...
def personal_dashboard(request):
    ...
    try:
        ...
        # Additional user-based data
        voice_data = VoicePattern.objects.filter(user=user).aggregate(Avg('stress_level'))['stress_level__avg'] or 0
        wearable_data = WearableData.objects.filter(user=user).aggregate(
            avg_heart_rate=Avg('heart_rate'),
            total_steps=Sum('steps'),
            avg_stress=Avg('stress_indicator')
        )
        
        # Feedback and Recommendations
        feedback_count = Feedback.objects.filter(user=user).count()
        recommendations = Recommendation.objects.filter(user=user, is_completed=False)[:5]
        alerts = Alert.objects.filter(user=user, is_acknowledged=False)[:5]
        ...
        context = {
            ...
            'voice_stress_avg': round(voice_data, 2) if voice_data else 0,
            'wearable_metrics': {
                'avg_heart_rate': round(wearable_data['avg_heart_rate'], 2) if wearable_data['avg_heart_rate'] else 0,
                'total_steps': wearable_data['total_steps'] or 0,
                'avg_stress': round(wearable_data['avg_stress'], 2) if wearable_data['avg_stress'] else 0,
            },
